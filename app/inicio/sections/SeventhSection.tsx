...
type EventCardProps = { item: EventItem; measureRef?: ((el: HTMLElement | null) => void) | RefObject<HTMLElement | null> };
...
const EventCard = memo(function EventCard({ item, measureRef }: EventCardProps) {
  return (
    <motion.article
      ref={measureRef}
...
export default function SeventhSection() {
  ...
  const step = useCallback((dir) => setIndex((i) => clamp(i + dir, 0, maxIndex)), [maxIndex]);
  const page = useCallback((dir) => setIndex((i) => clamp(i + perView * dir, 0, maxIndex)), [maxIndex, perView]);
