...
import { Newspaper, Search, Calendar, Maximize2 } from "lucide-react";
import ZoomModal from "../../components/ZoomModal";

const fadeUp = { hidden: { opacity: 0, y: 12 }, show: { opacity: 1, y: 0, transition: { duration: 0.28 } } };

// Imagem em alta indicada pelo usuário
const FULL_IMG = "https://res.cloudinary.com/diwvlsgsw/image/upload/v1762965931/images_2_wysfnt.jpg";

// MOCK — troque pela sua fonte/API real quando quiser
const ALL = [
  { slug: "o-operario-1913-05-12", title: "O Operário", date: "12/05/1913", decade: "1910s", cover: FULL_IMG, full: FULL_IMG, summary: "Organização de base e pautas salariais." },
  { slug: "folha-trabalhador-1921-09-03", title: "Folha do Trabalhador", date: "03/09/1921", decade: "1920s", cover: FULL_IMG, full: FULL_IMG, summary: "Mobilizações e comissões." },
  { slug: "gazeta-sindical-1932-02-28", title: "Gazeta Sindical", date: "28/02/1932", decade: "1930s", cover: FULL_IMG, full: FULL_IMG, summary: "Greves e negociação coletiva." },
...
export default function JornaisLanding() {
  ...
  const openModal = (it) => { setActive(it); setOpen(true); };
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-white/70 sm:text-lg">
            Clique em uma capa para abrir o leitor em tela cheia com zoom e arrastar. Use ↑/↓ para navegar na página aberta.
          </p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-12">
          ...
          <div className="md:col-span-4">
            <label className="sr-only" htmlFor="decade">Filtrar por década</label>
            <select
              id="decade"
              value={dec}
        ...
        {/* Grade de edições (cada card abre o modal de leitura) */}
            ...
            <article key={it.slug} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/60">
              ...
              <div className="p-3">
                ...
                <div className="mt-3 flex items-center gap-2 text-xs">
                  <Link href={`/jornais-de-epoca/${it.slug}`} className="rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-white/90 hover:bg-white/10">
        ...
        {/* Rodapé curto */}
        <p className="mt-8 text-center text-xs text-white/50">Para reprodução, consulte “Acesso à informação”. A leitura completa também está disponível na página da edição.</p>
      </motion.div>
    </section>
  );
}
